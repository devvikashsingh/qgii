 ...
 */
namespace app\modules\qgii\generators\crud;
...
class Generator extends \yii\gii\generators\crud\Generator
{
    ...
    public $viewPath;
    public $baseControllerClass = 'app\components\QController';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['modelClass' ,'baseControllerClass', 'indexWidgetType'], 'required'],
    ...
    public function generateActiveField($attribute)
    {
        ...
        if (preg_match('/^(created_on|updated_on|created_by_id)$/i', $column->name)) {
            return;
        }
        if (preg_match('/^(password|pass|passwd|passcode)$/i', $column->name)) {
            $input = 'passwordInput';
        }elseif(preg_match('/^(state_id|type_id|(.*?)_id)$/i', $column->name)) {
            $functionName = "get".Inflector::humanize($column->name)."List";
            return "\$form->field(\$model, '$attribute')->dropDownList(\$model->{$functionName}(), ['prompt' => ''])";
        }else {
            $input = 'textInput';
        }
    ...
    public function beforeValidate(){
        $modelClassName = (!empty($this->modelClass)) ? $this->modelClass : Inflector::id2camel($this->tableName, '_');
        $modelClassName = trim( str_replace(
            "app\\models\\",
            " ",
            " $modelClassName "
            ) );
        ...
        if (empty($this->searchModelClass))
            $this->searchModelClass = "{$this->ns}\\search\\{$modelClassName}";
        if (empty($this->controllerClass))
            $this->controllerClass = "app\\controllers\\{$modelClassName}Controller";
    ...
    public function autoCompleteData() {
        ...
        foreach(glob('./src/models/*.php') as $filename){
            ...
            $filename =  str_replace("./src", "app", $filename);
            $models [] =  str_replace("/", "\\", $filename);
        }
        return ['modelClass'=>$models];
    }
}
